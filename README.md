...
This git commit modifies the go compiler to support emitting warnings instead of errors when it encounters unused labels, variables, and imports. Warnings do not stop compilation or production of the final binary.

**All changes to the upstream go compiler are contained within this one commit.** From time to time, I'll rebase this commit onto upstream master and force-push to https://github.com/kstenerud/go master in order to keep it clean and current. To update to the latest version of this repo, you must first `git fetch`, `git fetch --tags`, and then `git reset --hard origin/master`. **Warning: this will destroy all your local changes!**
...
* [Rebasing to upstream](#rebasing-to-upstream)
...
To get the latest goodies from upstream and retain this patch:
